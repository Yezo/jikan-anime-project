...
import { Genre, Aired } from "../interfaces/interfaceJikanAPI";
...
const AnimeCard = ({
  ...
  return (
    <div
      className="flex h-[400px] flex-col rounded-md bg-white shadow-md ring-1 ring-black/[.05] sm:h-[275px]  sm:max-w-xl sm:flex-row"
      key={id}
    >
      {/* Left side */}
      <img
        className="h-full w-full rounded-t-md object-cover object-[top_center] sm:h-auto sm:w-48 sm:rounded-none sm:rounded-l-md"
        src={imageURL}
        alt={title}
      />
      {/* Right side */}
